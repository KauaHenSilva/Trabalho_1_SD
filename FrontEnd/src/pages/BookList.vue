                ...
                <div class="sidebar-content">
                    ...
                    <div class="filter-section">
                        ...
                        <div class="search-group">
                            <input type="text" v-model="searchTerm" placeholder="Buscar livros..." class="search-input"
                                :disabled="isLoading">
                            <button v-if="searchTerm" @click="clearSearch" class="clear-btn" title="Limpar busca">
                                ✕
                            </button>
                        </div>
            ...
            <main class="content-area">
                ...
                <div v-else class="main-content">
                                    ...
                                    <tr v-for="book in filteredBooks" :key="book.id" class="book-row">
                                        ...
                                        <td class="actions-cell">
                                            <div class="action-buttons">
                                                ...
                                                <router-link v-if="canEdit" :to="`/books/${book.id}/edit`"
                                                    class="action-btn-sm edit-btn" title="Editar">
                                                    ✏️
                                                </router-link>
                                                <button v-if="canDelete" @click="confirmDelete(book)"
                                                    class="action-btn-sm delete-btn" title="Excluir"
                            ...
                            <div v-for="book in filteredBooks" :key="book.id" class="book-card-horizontal">
                                ...
                                <div class="card-content">
                                    ...
                                    <div class="card-actions">
                                        <router-link :to="`/books/${book.id}`" class="card-btn card-btn-view">
                                            Ver Detalhes
                                        </router-link>
                                        <router-link v-if="canEdit" :to="`/books/${book.id}/edit`"
                                        ...
                                        <button v-if="canDelete" @click="confirmDelete(book)"
                                            class="card-btn card-btn-delete" :disabled="isDeleting === book.id">
                    ...
                    <div v-if="filteredBooks.length === 0 && !isLoading" class="empty-state">
                        ...
                        <h3>{{ searchTerm || categoryFilter || availabilityFilter ? 'Nenhum livro encontrado' :
                            'Biblioteca vazia' }}</h3>
                        <p>
                        ...
                        <router-link v-if="canCreate && !searchTerm && !categoryFilter && !availabilityFilter"
                            to="/books/create" class="empty-action-btn">
                            ➕ Adicionar Primeiro Livro
                        </router-link>
                        <button v-else-if="searchTerm || categoryFilter || availabilityFilter" @click="clearAllFilters"
...
const loadBooks = async () => {
    ...
    try {
        ...
        books.value = response as unknown as Book[]
    } catch (err) {
...
const deleteBook = async () => {
    ...
    try {
        ...
        cancelDelete()
        // Se não houve erro, recarrega os livros
        await loadBooks()
    } catch (err) {
        console.log(err)
...
// Watch para recarregar quando voltar para esta rota ou quando há mudanças na query
